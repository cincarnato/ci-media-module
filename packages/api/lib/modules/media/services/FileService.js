...
const findFile = async function (id) {
  return new Promise((resolve, reject) => {
    _FileModel.default.findOne({
      _id: id
    }).populate('createdBy').exec((err, res) => err ? reject(err) : resolve(res));
  });
};
...
const fetchFiles = async function () {
  return new Promise((resolve, reject) => {
    _FileModel.default.find({}).isDeleted(false).populate('createdBy').exec((err, res) => err ? reject(err) : resolve(res));
  });
};
